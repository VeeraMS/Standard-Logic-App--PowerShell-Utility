...
S.No  Parameter Name	   Mandatory	     Comments
    ...
    8   Workflow Name       Yes           Name of Workflow Name
	9	StartTime	        No	         If present all above operations will be performed on the runs started from this time.
				                           The Timestamp must be in UTC.				  
				                           Ex: 2020-11-02T16:33:00.000Z
	10	EndTime	            No             You can include the EndTime along with StartTime if you want to perform above operations between specific timestamps.
				                          Note:It is invalid without StartTime. 
...
param ([Parameter(Mandatory=$false)] $ClientId ,[Parameter(Mandatory=$false)] $TenantId,[Parameter(Mandatory=$false)] $Secret,[Parameter(Mandatory=$true)] $SubscriptionId,
[Parameter(Mandatory=$true)] $ResourceGroupName,[Parameter(Mandatory=$true)] $LogicAppName, [Parameter (Mandatory=$true)] $WorkFlowName,[Parameter (Mandatory=$true)] $Operation,
[Parameter(Mandatory=$false)] $StartTime ,[Parameter (Mandatory=$false)] $EndTime)
...
if(($starttime -ne $null) -and ($endtime -ne $null))
 {
 $url='https://management.azure.com/subscriptions/'+$subscriptionid+'/resourcegroups/'+$resourcegroupname+'/providers/microsoft.web/sites/'+$logicappname+'/hostruntime/runtime/webhooks/workflow/api/management/workflows/'+$workflowname+'/runs?api-version=2018-11-01&$filter=(status eq '''+$taskname+''' and starttime ge '+$starttime+' and starttime le '+$endtime+' )'
}
elseif(($starttime -ne $null) -and ($endtime -eq $null))
 {
$url='https://management.azure.com/subscriptions/'+$subscriptionid+'/resourcegroups/'+$resourcegroupname+'/providers/microsoft.web/sites/'+$logicappname+'/hostruntime/runtime/webhooks/workflow/api/management/workflows/'+$workflowname+'/runs?api-version=2018-11-01&$filter=(status eq '''+$taskname+''' and starttime ge '+$starttime+')'
}
else
{
$url='https://management.azure.com/subscriptions/'+$subscriptionid+'/resourcegroups/'+$resourcegroupname+'/providers/microsoft.web/sites/'+$logicappname+'/hostruntime/runtime/webhooks/workflow/api/management/workflows/'+$workflowname+'/runs?api-version=2018-11-01&$filter=(status eq '''+$taskname+''')'
}
...
$cancelrunurl= 'https://management.azure.com/subscriptions/'+$SubscriptionId+'/resourceGroups/'+$ResourceGroupName+'/providers/Microsoft.Web/sites/'+$LogicAppName+'/hostruntime/runtime/webhooks/workflow/api/management/workflows/'+$WorkFlowName+'/runs/'+$RunId+'/cancel?api-version=2018-11-01'
...
try
{
$Resubmiturl= 'https://management.azure.com/subscriptions/'+$SubscriptionId+'/resourceGroups/'+$ResourceGroupName+'/providers/Microsoft.Web/sites/'+$LogicAppName+'/hostruntime/runtime/webhooks/workflow/api/management/workflows/'+$WorkFlowName+'/triggers/'+$TriggerName+'/histories/'+$RunId+'/resubmit?api-version=2018-11-01'
...
do
{
# Check if its first iteration  of  REST call.
if($i -eq 0)
{
...
{
Write-Host 'No Runs found for the specified operation:' $Operation ', LogicApp Name:'$LogicAppName 'and WorkFlowName:' $WorkFlowName -ForegroundColor Yellow
}
}
}
else
